...
This repo contains all the code of the website from https://payment_gateway.com
...
1. Check that you have your .env.dev and .env.dev.db.  If for some reason these files are not generated, remember that you can find samples in the repository.
   ...
   DEBUG=1
   SECRET_KEY=}><O%WyI>|)GDc{WhqgH<]6*H598{B
   DJANGO_ALLOWED_HOSTS=localhost 127.0.0.1 [::1]
   SQL_ENGINE=django.db.backends.postgresql
   SQL_DATABASE=payment_gateway_dev
   SQL_USER=antoniahp
   SQL_PASSWORD=payment23
   SQL_HOST=db
   SQL_PORT=5432
   DATABASE=postgres
   ...
   POSTGRES_USER=antoniahp
   POSTGRES_PASSWORD=payment23
   POSTGRES_DB=payment_gateway_dev
    ```

2. Create docker volumes to save the data:
    ```sh
    $ docker volume create --name=payment_gateway_postgres_data_dev
    $ docker volume create --name=payment_gateway_grafana
    ```
...
* **Dockerhub:** You must create an [access token](https://docs.docker.com/docker-hub/access-tokens/) to download the Docker images from the production server. Then  you must create the following repositories in your Docker Hub account:
  * payment_gateway
  * payment_gateway-nginx

  * payment_gateway-prometheus
  * payment_gateway-grafana
 ...
 * **Environment files:** Once the server is created, you must copy the .env files in the production server inside **/opt/payment_gateway** path.
    ```sh
    $ cat .env.prod
    $ cat .env.prod.db
...
It will automatically load the provisioning configurations and serve the dashboard in [http://payment_gateway.com:3000](http://payment_gateway.com:3000).
